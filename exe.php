...
<?php
	...
	if ($eid != NULL){
		if($eid == '01' and in_array($get, $waiting) ){
			...
			if($result) echo '<script>alert("签到成功");window.location="index.php";</script>';
            else echo '<script>alert("权限不足或操作次序不当，导致操作失败！");history.go(-1);</script>';
		}elseif ($eid == '12' and in_array($get, $interview)) {
            $current_status = 1;
            $sql = sprintf("update record set status=%d where id=%d and status=%d", $current_status+1, $name, $current_status);
			$result = mysqli_query($config, $sql);
            if($result) echo '<script>alert("已让候场教室安排");window.location="index.php";</script>';
            else echo '<script>alert("权限不足或操作次序不当，导致操作失败！");history.go(-1);</script>';
            ...
            else echo '<script>alert("权限不足或操作次序不当，导致操作失败！");window.location="index.php";</script>';
		} else{
            echo '<script>alert("权限不足或操作次序不当，导致操作失败");window.location="index.php";</script>';
        }
		/*
		if($result){
		...
		*/
		...
		$result = mysqli_query($config, $sql);
		echo '<script>alert("写评论成功");window.location="index.php";</script>';
	}
